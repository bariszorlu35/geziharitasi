...
<body>
    ...
    <div class="container">
        <div class="header">
            ...
            <p>Onay bekleyen yorumları olan konumlar aşağıda listelenmiştir</p>
        </div>
        ...
        <div class="location-grid">
            ...
            $sorgu = $db->query("SELECT DISTINCT k.id, k.baslik, COUNT(y.id) as yorum_sayisi 
                                FROM konumlar k 
                                INNER JOIN yorumlar y ON k.id = y.marker_id 
                                WHERE y.durum = 0 
            ...
            foreach($sorgu->fetchAll(PDO::FETCH_ASSOC) as $row) {
                ...
                echo '<a href="yorumliste.php?id=' . $row["id"] . '&baslik=' . urlencode($row["baslik"]) . '" class="location-link">';
                ...
                echo '</div>';
                echo '<span class="comment-count">' . $row["yorum_sayisi"] . ' yorum</span>';
                echo '</a>';
                echo '</div>';
            }
            ?>
